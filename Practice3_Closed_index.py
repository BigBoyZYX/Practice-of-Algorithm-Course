...
def find_closed_index(collections, target):  # 查找最接近的目标的y下标
    n = len(collections)
    low = 0
    high = n - 1
    mid = 0
...
def divide(l, r, res):
    ...
    left = []  # 先将所有左子域、右子域、同一x值的中间子域，筛选出来
    right = []
    midd = []
    b = p[mid][0]
    bl = b - d
    ...
    for i in range(n):
        if ((p[i][0] >= bl) & (p[i][0] <= b)) == True:
            ...
            if p[i][0] == b:
                midd.append(p[i])
        elif ((p[i][0] <= br) and (p[i][0] >= b)) == True:
            right.append(p[i])

    if len(right) == 0:
        return d, res

    right.sort(key=lambda x: x[1])  # 将右子域先按照y值大小排好序

    for i in range(len(left)):   # 遍历左子域中的每一个点，在右子域中寻找y值最邻近的四个点，求出最小距离以及最近点对
        closed_point = []
        right_num = len(right)
        if right_num <= 4:
            closed_point = right
            ...
            closed_point.sort(key=lambda x: abs(x[1] - left[i][1]))  # 前四个就是右子域中离左子域最近的四个点
    ...
    if len(midd) > 1:  # 再在中间子域的内部进行比较，看看有没有x值相同，且距离最近的两个点
        midd.sort()
